...
var SPECIAL = /(gen[789](?:vgc20(?:19|21|22|23)|battlestadium(?:singles|doubles)|bs(?:s|d)))(.*)/;
var TRANSLATE = {
...
var Smogon = class _Smogon {
    ...
    for (const tierid in data.analyses) {
      ...
      for (const stub of a.sets) {
        ...
        if (set && this.match(species, this.toSet(species, set))) {
          analysis.sets.push(__spreadValues({
            name: stub.name,
  ...
  baseFormat(format) {
    ...
    return m ? TRANSLATE[m[1]] || m[1] : format;
  }
  ...
  name(gen, species, specific = false, stats = false) {
    var _a, _b;
    if (species.name.endsWith("-Crowned"))
      return species.name;
